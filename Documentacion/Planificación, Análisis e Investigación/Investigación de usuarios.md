# Encuesta de Investigación de Usuarios

## Objetivo:
Entender las necesidades, preferencias y desafíos de los usuarios que trabajan en proyectos de recetas de materiales sostenibles.

---

### 1. ¿Con qué frecuencia trabajas o experimentas con recetas de materiales sostenibles?
- [ ] Diariamente
- [ ] Semanalmente
- [ ] Mensualmente
- [ ] Raramente
- [ ] Nunca

---

### 2. ¿Qué tipo de proyectos o recetas sueles buscar o trabajar más frecuentemente?
Respuesta: _______________________________________________________________

---

### 3. ¿Qué desafíos o problemas has enfrentado al usar plataformas similares en el pasado?
Respuesta: _______________________________________________________________

---

### 4. ¿Qué herramientas o características te gustaría ver en nuestra plataforma que no hayas encontrado en otras?
Respuesta: _______________________________________________________________

---

### 5. ¿Qué tipo de tutoriales o recursos educativos te gustaría que estuvieran disponibles en la plataforma?
- [ ] Video tutoriales
- [ ] Guías escritas paso a paso
- [ ] Webinars o clases en vivo
- [ ] Foros de discusión
- [ ] Otro: ______

---

### 6. ¿Te gustaría tener la opción de colaborar con otros usuarios en proyectos o recetas?
- [ ] Sí
- [ ] No

---

### 7. ¿Qué tan importante es para ti que la plataforma tenga una aplicación móvil?
- [ ] Muy importante
- [ ] Importante
- [ ] Neutral
- [ ] Poco importante
- [ ] No importante

---

### 8. ¿Te gustaría que la plataforma ofreciera retos o concursos relacionados con la creación de recetas sostenibles?
- [ ] Sí
- [ ] No

---

### 9. ¿Qué características o funcionalidades consideras esenciales para una plataforma de recetas de materiales sostenibles?
Respuesta: _______________________________________________________________

---

### 10. ¿Hay alguna otra sugerencia o comentario que te gustaría compartir con nosotros?
Respuesta: _______________________________________________________________

---

### 11. ¿Cuál es tu nivel de experiencia en la creación y experimentación con recetas de materiales sostenibles?
- [ ] Principiante
- [ ] Intermedio
- [ ] Avanzado
- [ ] Experto

---

### 12. Al buscar recetas, ¿cuán importante es para ti la claridad y el detalle en las instrucciones?
- [ ] Muy importante
- [ ] Importante
- [ ] Neutral
- [ ] Poco importante
- [ ] No importante

---

### 13. En cuanto a la colaboración, ¿prefieres trabajar en proyectos:
- [ ] Individualmente
- [ ] En pequeños grupos
- [ ] En comunidades grandes
- [ ] No tengo preferencia

---

### 14. ¿Cuánto valoras la posibilidad de compartir tus propias recetas o modificaciones en la plataforma?
- [ ] Mucho
- [ ] Algo
- [ ] Poco
- [ ] Nada

---

### 15. ¿Cómo prefieres aprender sobre nuevas técnicas o materiales?
- [ ] Experimentando por mi cuenta
- [ ] Siguiendo guías paso a paso
- [ ] Viendo tutoriales en video
- [ ] Participando en foros de discusión
- [ ] Asistiendo a webinars o clases en vivo
- [ ] Otros: ____________________

---

### 16. ¿Con qué frecuencia te gustaría ver actualizaciones o nuevas recetas en la plataforma?
- [ ] Diariamente
- [ ] Semanalmente
- [ ] Mensualmente
- [ ] Ocasionalmente
- [ ] Solo cuando busco algo específico

---

### 17. Al considerar una receta, ¿qué factores son más importantes para ti? (Marca todas las que apliquen)
- [ ] Sostenibilidad del material
- [ ] Facilidad de preparación
- [ ] Costo de los ingredientes
- [ ] Disponibilidad de los ingredientes
- [ ] Tiempo requerido para la preparación
- [ ] Durabilidad del producto final
- [ ] Otro: ____________________

---

### 18. ¿Qué tan importante es para ti la presencia de una comunidad activa dentro de la plataforma?
- [ ] Muy importante
- [ ] Importante
- [ ] Neutral
- [ ] Poco importante
- [ ] No importante

---

### 19. ¿Cómo calificarías la importancia de las funciones de accesibilidad en la plataforma (como diseño responsivo, compatibilidad con lectores de pantalla, etc.)?
- [ ] Muy importante
- [ ] Importante
- [ ] Neutral
- [ ] Poco importante
- [ ] No importante

---

### 20. ¿Te gustaría que la plataforma ofreciera la posibilidad de obtener certificaciones o reconocimientos por tus proyectos o aprendizaje en el área de materiales sostenibles?
- [ ] Sí
- [ ] No

---

### 21. ¿Cuál es tu principal fuente de información o inspiración para encontrar recetas de materiales sostenibles?
- [ ] Sitios web especializados
- [ ] Redes sociales
- [ ] Libros o publicaciones impresas
- [ ] Comunidades o foros en línea
- [ ] Cursos o formación profesional
- [ ] Otro: ____________________

---

### 22. ¿Cuán importante es para ti la evaluación de impacto ambiental o la huella de carbono de los materiales utilizados en las recetas?
- [ ] Muy importante
- [ ] Importante
- [ ] Neutral
- [ ] Poco importante
- [ ] No importante

---

### 23. ¿Prefieres recetas que utilicen materiales fácilmente accesibles o aquellas que emplean materiales más especializados y únicos?
- [ ] Materiales fácilmente accesibles
- [ ] Materiales especializados y únicos
- [ ] Una combinación de ambos
- [ ] No tengo preferencia

---

### 24. ¿Cómo valoras la importancia de una sección de preguntas frecuentes (FAQ) y soporte en la plataforma?
- [ ] Muy importante
- [ ] Importante
- [ ] Neutral
- [ ] Poco importante
- [ ] No importante

---

### 25. ¿Te gustaría que la plataforma ofreciera opciones de personalización en tu perfil o área de trabajo, como temas, organización de recetas favoritas, etc.?
- [ ] Sí
- [ ] No

------

### 26. En términos de soporte y asistencia en la plataforma, ¿cómo prefieres recibir ayuda cuando enfrentas dificultades o tienes preguntas?
- [ ] Asistencia en tiempo real (chat en vivo)
- [ ] Foros de discusión
- [ ] Guías de ayuda o tutoriales
- [ ] Soporte por correo electrónico
- [ ] Preguntas frecuentes (FAQ)
- [ ] No necesito asistencia
- [ ] Otro: ____________________

---

### 27. ¿Cómo valoras la importancia de tener opciones de personalización en las recetas, como ajustar cantidades o sustituir ingredientes?
- [ ] Muy importante
- [ ] Importante
- [ ] Neutral
- [ ] Poco importante
- [ ] No importante

---

### 28. ¿Con qué frecuencia te gustaría recibir notificaciones sobre nuevas características, recetas o actualizaciones en la plataforma?
- [ ] Cada vez que haya una novedad
- [ ] Semanalmente
- [ ] Mensualmente
- [ ] Solo cuando inicie sesión en la plataforma
- [ ] Prefiero no recibir notificaciones

---

### 29. ¿Te interesa una función que te permita seguir a otros usuarios o ver sus contribuciones y proyectos en la plataforma?
- [ ] Sí
- [ ] No
- [ ] Indiferente

---

### 30. ¿Qué tan relevante es para ti que la plataforma sea accesible en múltiples idiomas?
- [ ] Muy relevante
- [ ] Algo relevante
- [ ] Neutral
- [ ] Poco relevante
- [ ] No relevante

---

